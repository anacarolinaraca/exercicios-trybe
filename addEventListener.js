...
const addClassTech = (event) => {
    const tagLi = document.querySelector('.tech');
    tagLi.classList.remove('tech');
...
input.addEventListener('input', (event) => {
    const mudarTexto = document.querySelector('.tech');
    mudarTexto.innerText = event.target.value;
})
...
myWebpage.addEventListener('dblclick', () => {
    window.location.replace('https://anacarolinaraca.github.io/');
});
...
const mudarCorDoTexto = (event) => {
  event.target.style.color = '#2fc18c';
}

const tirarCorDoTexto = (event) => {
    event.target.style.color = '';
}
myWebpage.addEventListener('mouseover', mudarCorDoTexto);
myWebpage.addEventListener('mouseout', tirarCorDoTexto);
